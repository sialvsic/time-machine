...
CategoryMoreController.prototype.getCategoryMore = (req, res, next) => {
  ...
  async.waterfall([
    (done)=> {
      Video.find({category: category}, done);
    }, (data, done)=> {
      length = data.length;
      var itemPerPage = constant.itemPerPage.movie;
      var skip = itemPerPage * (page - 1);

      var query = Video.find({category: category}, ('createTime title label description category screenshotsPath path'));
      //skip 跳过x个
      //limit 读取x个
    ...
    var respose = Object.assign({}, {doc: doc}, {allDatalength: length}, {category: category});
    res.send(respose);
  });
